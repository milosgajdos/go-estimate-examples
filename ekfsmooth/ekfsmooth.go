...
import (
	...
	"github.com/milosgajdos/go-estimate/smooth/erts"
...
func main() {
	...
	if err := window.Close(); err != nil {
		log.Fatal("Failed to close gocv window")
	}

	ic := sim.NewInitCond(estU[len(estU)-1].Val(), estU[len(estU)-1].Cov())

	s, err := erts.New(dot, ic, stateNoise)
	if err != nil {
		log.Fatal("Failed to create RTS smoother")
	}

	// we've got no input in this case
	estS, err := s.Smooth(estU, nil)
	if err != nil {
		log.Fatal("Failed to smooth filter output")
	}
	...
	for i := 0; i < len(estS); i++ {
		...
		DrawMarker(&img, pt, color.RGBA{0, 255, 0, 0}, 1)
	}

	window.IMShow(img)
	if window.WaitKey(0) == 27 {
		fmt.Printf("Shutting down: ESC pressed\n")
	}
}
